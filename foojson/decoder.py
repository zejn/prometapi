...
JS_VAR_MATCH = re.compile(r'([_$a-zA-Z][_$a-zA-Z0-9]*)()', FLAGS)
BACKSLASH = {
    '"': u'"', '\\': u'\\', '/': u'/',
    'b': u'\b', 'f': u'\f', 'n': u'\n', 'r': u'\r', 't': u'\t',
}
...
def py_scanstring(s, end, encoding=None, strict=True, _b=BACKSLASH, _m=STRINGCHUNK.match):
    ...
    while 1:
        ...
        if terminator == '"':
            break
        elif terminator == '' and _m == JS_VAR_MATCH.match:
            break
        elif terminator != '\\':
            if strict:
...
def JSONObject(slice_range, encoding, strict, scan_once, object_hook, _w=WHITESPACE.match, _ws=WHITESPACE_STR):
    ...
    if 1:
        ...
        if nextchar == '}':
            return pairs, end + 1
        #elif nextchar != '"' and _m:
        #    raise ValueError(errmsg("Expecting property name", s, end))
    #end += 1
    while True:
        key, end = scanstring(s, end, encoding, strict, _m=JS_VAR_MATCH.match)
